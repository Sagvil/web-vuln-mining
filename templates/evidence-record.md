...
- **API 文档页暴露即发现**（2026-08-25 妇院 N1 实战）：`/api/help`、Swagger、ASP.NET HelpPage 类页面未授权可达时，即使接口本身有认证，其参数规范与业务逻辑描述的公开本身就是 low 级信息泄露——重点记录"认证前提物"（如私钥分发机制）和危险业务语义（如自动建号）
- **前端壳与 API 面分离定性**：SPA 壳未授权可达 ≠ 漏洞；必须提取内嵌 API 清单并逐一验证认证状态后再定性（妇院 flexoffice：壳可达但 10 端点全 401 → info）
